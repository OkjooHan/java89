...
=> c프로그래밍 시절에는 리턴 값을 검사하여 예외를 처리하였다.
=> 리턴 값으로 호출한 쪽에 예외 상황을 알리는 방식의 한계
   - 예외 상황일 때 리턴하는 값이, 정상적인 작업에서 나올 수 있다.
     이런 경우를 처리할 수 없다.
   - 작업을 수행하는 명령과 오류를 처리하는 명령이 섞여있어서 유지보수가 힘들다.
     소스코드를 봤을 떄 어떤 일을 하는 코드인지 한 눈에 들어오지 않는다.
*/
package step14;
...
public class Exam080_1 {
  static int m1(int a, int b) {
    if (b == 0) {
      ...
      //어떻게? 리턴값으로.
      return -11119999; //계산값으로 나오지 않을 가능성이 있는 값을 리턴한다.
    }
    return a / b;
  }
