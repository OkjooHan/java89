/* 주제 : 제네릭의 정의와 사용
=> 제네릭은 타입 정보를 메서드의 파라미터처럼 클래스에 넘겨주는 기술이다.
   타입 이름을 받은 클래스는 내부에서 변수처럼 타입을 사용할 수 있다.
=> 제네릭 사용 이점
 1) 특정 데이터타입 별로 클래스 또는 메서드를 만들 필요가 없다.
 2) 특정 데이터타입에 종속되지 않고 모든 타입의 값을 다루려면 보통 Object 타입의
    레퍼런스를 사용하여 데이터를 다루게 되는데
    문제는,값을 꺼낼 떄마다 타입캐스팅(형변환) 할 필요가 없다.
    => 이것을 해결하기 위해 타입 이름을 파라미터처럼 클래스에 넘겨주는 제네릭이 등장하였다.
       즉, 제네릭을 사용하여 타입캐스팅 할 필요가 없다.
...
public class Test  {
  ...
  //그 클래스가 다루는 데이터타입의 이름을 파라미터로 넘겨줘야 한다.
